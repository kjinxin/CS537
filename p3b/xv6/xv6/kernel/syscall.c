...
{
  if((addr >= p->sz && addr < p->shmtop) || (addr+4 > p->sz && addr + 4 <= p->shmtop) 
...
fetchstr(struct proc *p, uint addr, char **pp)
{
  ...
  if((addr >= p->sz && addr < p->shmtop) || addr >= USERTOP)
    return -1;
  *pp = (char*)addr;
  ep = (char*)p->sz;
  if (addr >= p->sz) {
...
argptr(int n, char **pp, int size)
{
  ...
  if(argint(n, &i) < 0)
    return -1;
  if(((uint)i >= proc->sz && (uint)i < proc->shmtop) || ((uint)i+size > proc->sz && (uint)i + size < proc->shmtop)
