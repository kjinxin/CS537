...
// first use the key, we will allocate physical memory for it
int shmalloc(pde_t *pgdir, uint shmtop, int key, int value) {
  char *mem;
  uint a;
  int i;
  a = shmtop - value * PGSIZE;
  for(i = 0; i < value; i ++, a += PGSIZE){
    mem = kalloc();
    if(mem == 0){
      return 0;
    }
    memset(mem, 0, PGSIZE);
    shms[key].pa[i] = (void*) mem;
...
int shmmap(pde_t *pgdir, uint shmtop, int key, int value) {
  uint a;
  int i;
  a = shmtop - value * PGSIZE;
  for(i = 0; i < value; i ++, a += PGSIZE){
    mappages(pgdir, (char*)a, PGSIZE, PADDR(shms[key].pa[i]), PTE_W|PTE_U);
  }
  return 0;
}
...
freevm(pde_t *pgdir)
{
  ...
  deallocuvm(pgdir, proc->shmtop, 0);
