...
bmap(struct inode *ip, uint bn)
{
  ...
  if(bn < NDIRECT){
    ...
    if (ip->type == T_CHECKED) {
      bp = bread(ip->dev, addr & MASK2);
      uint checksum = 0, i;
      for (i = 0; i < BSIZE; i ++) {
        checksum ^= bp->data[i];
      }
      
      ip->addrs[bn] = addr = ((addr & MASK2) | (checksum << 24));
      brelse(bp);
    }
  ...
  if(bn < NINDIRECT){
    ...
    if (ip->type == T_CHECKED) {
      uint checksum = 0, i;
      struct buf *bp2;
      bp2 = bread(ip->dev, addr & MASK2);
      for (i = 0; i < BSIZE; i ++) {
        checksum ^= bp2->data[i];
      }
      a[bn] = addr = (addr & MASK2) | (checksum << 24);
      bwrite(bp);
      brelse(bp2);
    }
